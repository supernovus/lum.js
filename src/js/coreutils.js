...
//        console.log("copying properties", source);
  ...
  /**
   * Parse a URL hash into a pseudo-query string.
   *
   ...
   *   'separate'  RegExp to use for option split (default: ';').
   *   'assign'    RegExp to use for assignment split (default: '=').
   *   'true'      RegExp to use for true values (default: /^(true|yes)$/i).
   *   'false'     RegExp to use for false values (default: /^(false|no)$/i).
   *   'json'      RegExp to use for JSON values (default: false, see below.)
   *   'source'    A string to use instead of location.hash (debugging only.)
   *
   * If an option does not have an assignment, it's value will be set to null.
   *
   * Any of the 'json', 'true', or 'false' options can be set to false or null
   * to disable the parsing of those values.
   *
   * The 'json' parameter can either be a RegExp, or one of the following
   * values which correspond to the build in RegExps:
   *
   *   true      /^[\{[].*?[\}\]]$/    Match both arrays and objects.
   *   'array'   /^\[.*?\]$/           Match only arrays.
   *   'object'  /^\{.*?\}$/           Match only objects.
   *
   */
  Nano.getHashOpts = function (getOpts={})
  {
    var hashOpts = (typeof getOpts.defaults === 'object' && getOpts.defaults !== null) ? Nano.clone(getOpts.defaults) : {};

    var srcHash = 'source' in getOpts ? getOpts.source : location.hash;

    if (srcHash && srcHash != '#')
    { // We have a hash, let's split it up.

      var sep = getOpts.separate ? getOpts.separate : ';';
      var ass = getOpts.assign ? getOpts.assign : '=';

      var tv = 'true' in getOpts ? getOpts.true : /^(true|yes)$/i;
      var fv = 'false' in getOpts ? getOpts.false : /^(false|no)$/i;
      var jc = false;

      if ('json' in getOpts)
      {
        if (typeof getOpts === 'object' && getOpts instanceof RegExp)
        {
          jc = getOpts.json;
        }
        else if (getOpts.json === true)
        {
          jc = /^[\{\[].*?[\}\]]$/;
        }
        else if (getOpts.json === 'array')
        {
          jc = /^\[.*?\]$/;
        }
        else if (getOpts.json === 'object')
        {
          jc = /^\{.*?\}$/;
        }
      }
      ...
      var hash = srcHash.substr(1); // take off the '#' symbol.
      var segments = hash.split(sep);
      for (var s = 0; s < segments.length; s++)
      {
        var segment = segments[s];
        var assignment = segment.split(ass);
   ...
   *   'forceNamed'  Set to true if we require named options (see below).
   *
   ...
   * and the single option was the value itself. You can disable this
   * feature by setting the 'forceNamed' option to true.
   *
   * Note that the getOpts are passed to Nano.getHashOpts() as well, so any
   * options applicable to that method may also be specified.
   */
  Nano.getHashOpt = function (name, getOpts={})
  {
    var hashOpts = Nano.getHashOpts(getOpts);
    ...
    {
      var keys = Object.keys(hashOpts);
      if (!getOpts.forceNamed && keys.length == 1 && hashOpts[keys[0]] === null)
